...
jx.Controler.prototype.update = function(deltaTime) {
  if (this.type == 'keyboard') {
    ...
    this.DOWN  = this.gamepad[hash.DOWN]  > 0 ? Math.abs(this.gamepad[hash.DOWN])  : 0;
    console.log(this.gamepad, this.DOWN);
    this.UP    = this.gamepad[hash.UP]    < 0 ? Math.abs(this.gamepad[hash.UP])    : 0;
    this.LEFT  = this.gamepad[hash.LEFT]  < 0 ? Math.abs(this.gamepad[hash.LEFT])  : 0;
    this.RIGHT = this.gamepad[hash.RIGHT] > 0 ? Math.abs(this.gamepad[hash.RIGHT]) : 0;
